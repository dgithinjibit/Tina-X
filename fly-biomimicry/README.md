# Fly Biomimicry — Borrowing the Fly's Design for Nzi

> "Nzi" = fly (Swahili). This folder captures the *biological traits of flies* that have real
> robotics implementations, and maps each to a concrete Nzi design decision.
...
- **Nzi:** fast inner attitude loop on low-latency rate gyros; budget the reflex loop at single-digit-to-~13 ms, below actuation frequency.
...
- **Nzi:** delayed-PD stabilizer as the low-level primitive per agent. **Keep MeTTa/Hyperon symbolic
...
- **Nzi:** lightweight optic-flow obstacle avoidance + multimodal fusion → less reliance on GPS/precise
...
- **Nzi:** design for graceful physical robustness + simple obstacle-agnostic controllers at swarm scale.
...
- **Nzi:** mechanical/passive stability offloads the control stack; resilient actuation = durability.
- **HONEST CAVEAT:** the MIT robot is still **tethered to wired power** (actuators need high voltage).
  ...
  scales without solving the voltage/power problem. → This determines Nzi's realistic minimum node size.

---

## Design DNA summary (fly → Nzi)
